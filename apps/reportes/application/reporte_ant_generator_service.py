...
class ReporteANTPDFBuilder:
    ...
    def __init__(
        self, periodo_nombre, generado_por_nombre, generado_por_cedula, firma_path=None, notas=""
    ):
    ...
    def _elementos(self, estudiantes, totales, hash_footer):
        ...
        elements.append(Paragraph(f"Período académico: {self.periodo_nombre}", s["Normal"]))
        elements.append(
            Paragraph(
        ...
        for d in estudiantes:
            promedio_str = f"{d.promedio_final:.2f}" if d.promedio_final is not None else "—"
            rows.append(
                [
                    d.cedula,
                    d.nombres_completos,
